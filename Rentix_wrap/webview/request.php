...
$postDataJSON = '{"value":604,"app_id":"9999990088","cmd":"customer_phone_data","customer_id":"583","acc_gmail":"koltykov@gmail.com","acc_vk":"Анатолий Колтыков","token":"dkdtDwq4ZE0:APA91bE1t-UugVUie853aCG3fLSB52zLmGZXD2FLr7lB4A8qsMSmjppDK7MrbOK0y_OhaVzLBPVxjy2H5soJRkdsdZCrTl1AsHPvXZhj28B_Hesc8wni-Rj3HxYjB26f9hDsYDsH5UpJ"}';
*/
...
switch ($cmd){
    ...
    case 'rent_start':
    	...
    	log_command($DataJSON, $app_id, $cmd);
    	save_ping($app_id, $value);
    break;
    case 'rent_pause':
    	rent_update($DataJSON, $app_id);
    ...
    case 'delete_product':
    	...
    	save_ping($app_id, $value);
    break;
    case 'customer_phone_data':
    	customer_phone_data($DataJSON, $app_id);
    break;    
...
function customer_phone_data ($DataJSON, $app_id){

	$customer_id = 	filter_var($DataJSON->{'customer_id'}, FILTER_VALIDATE_INT);				// Номер клиента (нумерация серверная)
	$acc_vk = 			filter_var($DataJSON->{'acc_vk'}, FILTER_SANITIZE_STRING);					// VK
	$acc_gmail = 		filter_var($DataJSON->{'acc_gmail'}, FILTER_SANITIZE_STRING);				// GMail
	$token = 				filter_var($DataJSON->{'token'}, FILTER_SANITIZE_STRING);						// Токен для Firebird Cloud Messaganing
			
	$pDB = rent_connect_DB();
	
	$sql = 'SELECT * FROM `clients` WHERE `id` = '.$customer_id.' AND `id_rental_org` = '.$app_id;		// Проверяем на существование клиента
	$cust = $pDB->get($sql, true, false);	
	
	$sql = 'SELECT * FROM `client_data` WHERE `token` = "'.$token.'" AND `id_rental_org` = '.$app_id;		// Проверяем на существование такого токена
	$tn = $pDB->get($sql, true, false);	
	$token_id = $tn['id'];
	
	if(!$tn && $cust){						// Если токен еще не найден в базе и такой клиент существует, то сохраняем токен
		$sql = 'INSERT INTO `client_data` (`id`, `id_rental_org`, `id_customer`, `acc_gmail`, `acc_vk`, `token`, `ip`, `updated`) 
														VALUES (NULL, :app_id, :id_customer, :acc_gmail, :acc_vk, :token, :ip, NOW())';							
														
		$pDB->set($sql, array('app_id' => $app_id, 'id_customer' => $customer_id, 'acc_gmail' => $acc_gmail, 'acc_vk' => $acc_vk, 'token' => $token, 'ip' => $_SERVER['REMOTE_ADDR']));
		if(is_int($pDB->getLastId())) {
			$token_id = $pDB->getLastId();
		}
	}
	$ot = array('token_id' => $token_id, 'customer_phone' => $cust[phone]);
	out($ot);
}
...
function log_command($DataJSON, $app_id, $cmd){
	$order = filter_for_order ($DataJSON);
	$pDB = rent_connect_DB();
	$info = 'order ID: '.$order['order_id'].'; Bill: '.$order[bill].'; Client: '. $order['customer_name'];
	$sql = 'INSERT INTO `log_command` (`server_time`, `id_rental_org`, `ip`, `command`, `info`) VALUES (now(), :app_id, :ip, :command, :info)';																	
	$pDB->set($sql, array('app_id' => $app_id, 'ip' => $_SERVER['REMOTE_ADDR'], 'command' => $cmd, 'info' => $info));
}
